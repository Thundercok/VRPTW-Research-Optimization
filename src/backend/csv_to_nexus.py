...
BKS: dict[str, dict[str, float]] = {
    ...
    "C204":{"nv":3,"td":590.6},"C205":{"nv":3,"td":588.88},"C206":{"nv":3,"td":588.49},
    "C207":{"nv":3,"td":588.29},"C208":{"nv":3,"td":588.32},
    "R101":{"nv":19,"td":1650.8},"R102":{"nv":17,"td":1486.12},"R103":{"nv":13,"td":1292.68},
    ...
    "R201":{"nv":4,"td":1252.37},"R202":{"nv":3,"td":1191.7},"R203":{"nv":3,"td":939.5},
...
def _summary_row(r: dict, label: str) -> dict[str, Any]:
    return {
        "algo": label, "instance": r["Instance"], "dataset": r["Dataset"],
...
def _transfer_row(r: dict) -> dict[str, Any]:
    return {
        "instance": r["Instance"], "dataset": r["Dataset"], "algo": r["Algorithm"],
        "gap_pct": _f(r.get("Gap%","")), "nv": _f(r.get("NV_mean","")),
        "td":      _f(r.get("TD_mean","")), "time_s": _f(r.get("Time_s","")),
        "nv_diff": _f(r.get("NV_diff","")),
        "nv_inflated": r.get("NV_inflated","False").strip().lower() == "true",
    }
...
def build_nexus(rows: list[dict], version: str) -> dict[str, Any]:
    ...
    for r in rows:
        ...
        (transfer if is_tr else summary).append(
            _transfer_row(r) if is_tr else _summary_row(r, label)
        )
