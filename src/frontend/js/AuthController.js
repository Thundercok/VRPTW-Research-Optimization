...
export class AuthController {
  ...
  wireEvents() {
    ...
    el.btnOpenRegister?.addEventListener('click', (e) => {
      e.preventDefault();
      this.showAuthView('register');
    });
    el.linkForgotPassword?.addEventListener('click', (e) => {
      e.preventDefault();
      this.showAuthView('forgot');
    });
    el.btnBackLoginFromRegister?.addEventListener('click', (e) => {
      e.preventDefault();
      this.showAuthView('login');
    });
    el.btnBackLoginFromForgot?.addEventListener('click', (e) => {
      e.preventDefault();
      this.showAuthView('login');
    });
    el.btnBackLoginFromReset?.addEventListener('click', (e) => {
      e.preventDefault();
      this.showAuthView('login');
    });
    el.btnRequestOtp?.addEventListener('click', (e) => {
      e.preventDefault();
      this.requestRegisterOtp();
    });
    el.btnRegister?.addEventListener('click', (e) => {
      e.preventDefault();
      this.register();
    });
    ...
    el.btnForgotPassword?.addEventListener('click', (e) => {
      e.preventDefault();
      this.requestForgotPasswordOtp();
    });
    el.btnResetPassword?.addEventListener('click', (e) => {
      e.preventDefault();
    ...
    if (view === 'register') {
      if (this.app.state.registerOtpExpiresAt > Date.now()) {
        ...
        this.updateRegisterOtpCountdownText(
          this.app.lang === 'vn' ? 'Bấm Gửi OTP để nhận mã xác thực.' : 'Click Send OTP to receive a verification code.'
        );
      }
      this.updateRegisterButtonState();
    } else {
  ...
  scheduleRealtimeOtpVerification() {
    ...
    const state = this.app.state;
    const canVerify =
      state.registerOtpApprovedEmail === email && state.registerOtpExpiresAt > Date.now() && /^\d{6}$/.test(otp);
    if (!canVerify) return;
