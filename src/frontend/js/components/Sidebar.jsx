  ...
  return (
    <aside className={`saas-sidebar ${collapsed ? 'collapsed' : ''}`} onDoubleClick={toggleCollapse}>
      ...
      <nav className="saas-nav">
        <div className="nav-group nav-group-planning">
          {collapsed ? (
            <div className="nav-group-rule" aria-hidden="true" />
          ) : (
            <p className="nav-label">
              <span className="nav-label-dot" aria-hidden="true" />
              <span className="nav-label-text">{t('sbPlanning')}</span>
            </p>
          )}
          <a
            href="#"
            data-tour="nav-dispatch"
        ...
        <div className="nav-group nav-group-intelligence">
          {collapsed ? (
            <div className="nav-group-rule" aria-hidden="true" />
          ) : (
            <p className="nav-label">
              <span className="nav-label-dot" aria-hidden="true" />
              <span className="nav-label-text">{t('sbIntelligence')}</span>
            </p>
          )}
          <a
            href="#"
      ...
      <div className="sidebar-footer">
        <a href="index.html" className={collapsed ? 'btn-icon-only' : 'btn-back-home'} title={t('sbWebsite')}>
          <svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
        ...
        {state.unlocked && state.role !== 'guest' ? (
          <>
            ...
            <button
              id="btn-logout"
              className={collapsed ? 'btn-icon-only' : 'btn-text'}
              onClick={logout}
