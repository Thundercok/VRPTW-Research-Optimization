...
\setlength{\headheight}{119.6pt}%
\addtolength{\topmargin}{-15.6pt}%
...
Bài toán định tuyến phương tiện (VRP) được giới thiệu lần đầu bởi Dantzig và Ramser (1959). Kể từ đó, hàng loạt biến thể của bài toán đã được phát triển nhằm đáp ứng các yêu cầu thực tiễn, trong đó VRPTW là biến thể phổ biến nhất do tính thực tế cao của các ràng buộc về mặt thời gian (Solomon, 1987) \cite{solomon1987}.
...
Bài toán được phát biểu trên đồ thị định hướng đầy đủ $G = (V, A)$ (minh họa tại Hình~\ref{fig:vrptw_clean}). Đội xe đồng nhất gồm $K$ phương tiện, mỗi phương tiện có sức tải tối đa $Q$. Các ký hiệu và biến quyết định được mô tả như sau:
...
\node[brain] (SM) at (0, 3.3) {%
  \textbf{Bộ giám sát trì trệ}\\
  {\color{C_rose!70}\scriptsize\textit{in:}} success flag, $NV_t$, $NV^*$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} bộ đếm \texttt{no\_imp}, trigger};

\node[brain] (PC) at (0, 0.8) {%
  \textbf{Plateau Controller (Macro DDQN)}\\
  {\color{C_rose!70}\scriptsize\textit{in:}} trạng thái $s_t^c \in \mathbb{R}^{12}$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} chế độ tìm kiếm $m \in \{0..5\}$};

\node[brain] (OC) at (0, -1.8) {%
  \textbf{Operator Controller (Micro DDQN)}\\
  {\color{C_rose!70}\scriptsize\textit{in:}} trạng thái $s_t^o \in \mathbb{R}^{15}$, mục tiêu $g_m$, $m$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} cặp toán tử $a_t = (d, r)$};

% Column 2: Muscle (ALNS Search & MIP Recombination)
\node[muscle] (AE) at (6.2, 2.0) {%
  \textbf{Bộ máy ALNS Core Engine}\\
  {\color{C_indigo!70}\scriptsize\textit{in:}} cặp $(d, r)$, lời giải $x_t$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} lời giải ứng viên $x'$, tập $\mathcal{R}$};

\node[muscle] (LA) at (6.2, -0.5) {%
  \textbf{Bộ lọc chấp nhận (LAC MLP)}\\
  {\color{C_indigo!70}\scriptsize\textit{in:}} đặc trưng $[\Delta c, T_t, \dots] \in \mathbb{R}^9$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} bit chấp nhận $b_t$, nhãn $y_t$};

\node[muscle] (EV) at (6.2, -3.0) {%
  \textbf{Tái tổ hợp tuyến đường MIP}\\
  {\color{C_indigo!70}\scriptsize\textit{in:}} chấp nhận $x'$, tập cột tuyến $\Omega$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} kỷ lục $x^*$, phần thưởng thô $r_t$};

% Column 3: Memory & Learning
\node[memory] (RP) at (12.4, 2.0) {%
  \textbf{Kho lưu trữ tuyến (Route Pool)}\\
  {\color{C_amber!80}\scriptsize\textit{in:}} tuyến từ $x'$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} tập cột $\Omega$ cho Set Partitioning};

\node[memory] (PER) at (12.4, -0.5) {%
  \textbf{Bộ đệm trải nghiệm (PER)}\\
  {\color{C_amber!80}\scriptsize\textit{in:}} chuyển đổi $(s, a, z, s', d)$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} batch huấn luyện ưu tiên};

\node[learning] (WN) at (12.4, -3.0) {%
  \textbf{Chuẩn hóa phần thưởng Welford}\\
  {\color{C_emerald!80}\scriptsize\textit{in:}} phần thưởng thô $r_t \in \mathbb{R}$\\
  {\color{C_slate!70}\scriptsize\textit{out:}} phần thưởng chuẩn hóa $z_t \in [-8, +8]$};

% Group labels (subgraph boxes in TikZ using fit or background)
\begin{scope}[on background layer]
  \node[draw=C_rose!40, dashed, fill=C_rose!2, line width=0.8pt, rounded corners=5pt, fit=(SM) (PC) (OC), inner sep=8pt, label={[font=\sffamily\bfseries\scriptsize, text=C_rose!90]above:NÃO BỘ QUYẾT ĐỊNH (DRL)}] {};
  \node[draw=C_indigo!35, dashed, fill=C_indigo!2, line width=0.8pt, rounded corners=5pt, fit=(AE) (LA) (EV), inner sep=8pt, label={[font=\sffamily\bfseries\scriptsize, text=C_indigo!90]above:CƠ BẮP TÌM KIẾM (ALNS + MIP)}] {};
  \node[draw=C_emerald!35, dashed, fill=C_emerald!2, line width=0.8pt, rounded corners=5pt, fit=(RP) (PER) (WN), inner sep=8pt, label={[font=\sffamily\bfseries\scriptsize, text=C_emerald!90]above:HỌC HỎI \& PHẢN HỒI}] {};
\end{scope}

% Flow arrows with color-coded sequence badges (Vietnamese labels)
\draw[flow] (SM) -- node[badge_rose, left=2pt] {1} node[lbl, right=2pt] {\texttt{no\_imp} $\ge 60$} (PC);
\draw[flow] (PC) -- node[badge_rose, left=2pt] {2} node[lbl, right=2pt] {chế độ $m$} (OC);
\draw[flow] (OC.east) -- (3.3, -1.8) -- node[badge_rose, pos=0.3, left=2pt] {4} node[lbl, pos=0.6, left=2pt] {cặp $(d, r)$} (3.3, 2.0) -- (AE.west);

\draw[flow] (AE) -- node[badge_indigo, left=2pt] {5} node[lbl, right=2pt] {ứng viên $x'$} (LA);
\draw[flow] (LA) -- node[badge_indigo, left=2pt] {7} node[lbl, right=2pt] {chấp nhận $x'$} (EV);
\draw[flow] (EV) -- node[badge_amber, pos=0.3, below=2pt] {9} node[lbl, pos=0.7, below=2pt] {phần thưởng $r_t$} (WN);

\draw[flow] (AE.east) -- node[badge_indigo, pos=0.3, above=2pt] {6} node[lbl, pos=0.7, above=2pt] {tuyến hợp lệ} (RP.west);
\draw[flow] (RP.west) -- (9.3, 2.0) -- node[badge_amber, pos=0.3, right=2pt] {8} node[lbl, pos=0.65, right=2pt] {cột tuyến $\Omega$} (9.3, -3.0) -- (EV.east);

\draw[flow] (WN) -- node[badge_emerald, left=2pt] {10} node[lbl, right=2pt] {thưởng $z_t$} (PER);

% Feedback paths (rerouted with color-coded sequence badges)
\draw[back] (PER.west) -- (9.0, -0.5) -- (9.0, -1.8) -- node[badge_emerald, pos=0.3, above=2pt] {11} node[lbl, pos=0.7, above=2pt] {batch huấn luyện} (OC.east);

\draw[back] (WN.south) -- (12.4, -4.15) -- node[badge_emerald, pos=0.25, above=2pt] {12} node[lbl, pos=0.5, above=2pt] {thưởng $z_t$} (-2.5, -4.15) -- (OC.west);
\draw[back] (WN.south) -- (12.4, -4.5) -- node[badge_emerald, pos=0.25, above=2pt] {13} node[lbl, pos=0.5, above=2pt] {sai số TD $\delta$} (-3.1, -4.5) -- (-3.1, 0.8) -- (PC.west);

% Goal conditioning with color-coded sequence badge
\draw[goal] (PC.south east) -- node[badge_rose, pos=0.35, left=2pt] {3} node[lbl, pos=0.75, left=2pt, text=C_fuchsia!90] {mục tiêu $g_m$} (OC.north east);

% ── Chú giải (Embedded legend) ─────────────────────────────────────────────
\node[draw=C_slate!20, fill=white, rounded corners=3pt, inner sep=5pt,
      font=\sffamily\tiny] (leg) at (6.2, -5.6) {};
\draw[flow]  (-0.5, -5.6) -- ++(0.7, 0)
  node[right, font=\sffamily\tiny, text=C_slate!80] {Luồng tiến};
\draw[back]  (2.1, -5.6)  -- ++(0.7, 0)
  node[right, font=\sffamily\tiny, text=C_slate!80] {Phản hồi};
\draw[goal]  (4.8, -5.6)  -- ++(0.7, 0)
  node[right, font=\sffamily\tiny, text=C_slate!80] {Tín hiệu mục tiêu};
...
\node[font=\sffamily\tiny, text=C_slate!80, right=3pt] at (8.6, -5.6) {ALNS};
...
\node[font=\sffamily\tiny, text=C_slate!80, right=3pt] at (11.2, -5.6) {Học hỏi};
\end{tikzpicture}%
}
\caption{Kiến trúc Hierarchical MDP temporally-coupled của hệ thống Hybrid DDQN-ALNS, kết hợp các phân vùng Não bộ Quyết định (DRL), Cơ bắp Tìm kiếm (ALNS + MIP Recombination) và Cơ chế Học hỏi \& Phản hồi trong một vòng lặp học--tối ưu đồng bộ.}
...
Mạng nơ-ron sử dụng kiến trúc Dueling Double DQN \cite{wang2016} với hàm giá trị Q được biểu diễn qua công thức:
\begin{equation}
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item Thư viện phát triển: PyTorch 2.1, SciPy 1.11, Python 3.10. Số vòng lặp tối đa: 5000 vòng lặp. 
\end{itemize}
...
\begin{table}[!htbp]
\centering
\caption{So sánh tổng hợp hiệu năng trung bình theo lớp dữ liệu Solomon với cấu hình 5000 vòng lặp}
...
\midrule
\multirow{5}{*}{RC1} & ALNS-Base & 12.035 & 1373.89 & -0.434\% & 50.2 \\
 & Hybrid-Fixed & 11.911 & 1358.82 & -1.556\% & 83.2 \\
 & Hybrid-Rule & 11.875 & 1359.25 & -1.530\% & 66.4 \\
 & \textbf{Hybrid-DDQN (Đề xuất)} & \textbf{11.875} & \textbf{1358.89} & \textbf{-1.555\%} & \textbf{74.1} \\
 & OR-Tools & 13.000 & 1418.64 & +2.921\% & 120.3 \\
\midrule
\multirow{5}{*}{RC2} & ALNS-Base & 3.375 & 1114.34 & +1.860\% & 22.1 \\
 & Hybrid-Fixed & 3.250 & 1137.10 & +3.774\% & 42.4 \\
 & Hybrid-Rule & 3.250 & 1130.09 & +3.160\% & 33.9 \\
 & \textbf{Hybrid-DDQN (Đề xuất)} & \textbf{3.250} & \textbf{1128.05} & \textbf{+2.976\%} & \textbf{39.2} \\
 & OR-Tools & 3.625 & 1157.91 & +6.346\% & 120.1 \\
\bottomrule
\end{tabular}}
\end{table}
