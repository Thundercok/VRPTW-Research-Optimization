...
\begin{minipage}{0.82\textwidth}
    ...
    \textbf{SINH VIÊN/NHÓM SINH VIÊN THỰC HIỆN:} \\[0.15cm]
    \hspace*{0.5cm} \textbf{1.} \textit{HUỲNH NHẬT HUY} \\[0.1cm]
    \hspace*{0.5cm} \textbf{2.} \textit{NGUYỄN NHẬT HUY}
\end{minipage}
\vfill
\begin{center}
    \fontsize{13pt}{16pt}\selectfont
...
\textbf{So với Hottung và Tierney (2019, 2020):}
Phương pháp \textit{Neural Large Neighborhood Search (NLNS)} và \textit{Deep Policy Dynamic Programming (DPDP)} của Hottung \& Tierney tập trung vào việc sử dụng mạng thần kinh (attention-based) để trực tiếp sinh lộ trình hoặc sinh các quyết định phá hủy/sửa chữa ở cấp độ nút (node-level). Tuy nhiên, hướng tiếp cận này gặp khó khăn rất lớn khi đối mặt với các bài toán có ràng buộc khung thời gian cứng và ngặt nghèo (Time Windows) do không gian lời giải hợp lệ bị co hẹp đáng kể, dẫn đến việc mạng nơ-ron dễ sinh ra các lộ trình không khả thi hoặc tốn kém thời gian sửa đổi. Ngược lại, mô hình lai \textbf{DDQN-ALNS} đề xuất sử dụng DRL ở cấp độ điều khiển chiến lược (chọn chế độ tìm kiếm và chọn cặp toán tử heuristic từ thư viện có sẵn). Việc giữ nguyên các toán tử sửa chữa heuristic giúp đảm bảo tính khả thi tuyệt đối của lời giải về mặt toán học thông qua các bộ lọc ràng buộc nhanh, trong khi vẫn tận dụng được khả năng học hỏi thông minh từ mạng nơ-ron để tăng tốc độ hội tụ.

\textbf{So với Bi và đồng tác giả (2022):}
...
\caption{Kiến trúc Hierarchical MDP temporally-coupled của hệ thống Hybrid DDQN-ALNS, kết hợp Plateau Controller, Operator Controller, ALNS Core Engine, LAC và Welford Normalizer trong một vòng lặp học--tối ưu.}
...
\begin{itemize}[leftmargin=1.2cm]
  \item CPU: Intel Core i7-14700KF (28 nhân, tốc độ tối đa 5.6 GHz). RAM: 32 GB DDR5.
...
\begin{table}[!htbp]
\centering
\caption{So sánh tổng hợp hiệu năng trung bình theo lớp dữ liệu Solomon}
...
\begin{table}[!htbp]
\centering
\caption{Hiệu năng của mô hình chuyển giao đóng băng trọng số (Transfer-DR)}
...
\begin{itemize}[leftmargin=1.2cm]
  ...
  \item Đạt kết quả cạnh tranh trên tập Solomon với Gap\% chỉ 0.16\%, vượt qua giải pháp thương mại Google OR-Tools về mặt kinh tế (Fleet Size).
...
\bibitem{sambrook2001}
Sambrook, J. and D.W. Russell. 2001. \textit{Molecular Cloning: A Laboratory Manual}. New York: Cold Spring Harbor Laboratory Press.
