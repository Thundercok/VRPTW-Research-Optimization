...
% ── Figure 3: Detailed architecture block diagram ────────────────────────────
\begin{figure*}[!t]
\centering
\begin{tikzpicture}[
    font=\sffamily\footnotesize, >=Stealth,
    ...
    det/.style={rectangle, rounded corners=4pt, semithick, align=center,
                minimum width=7.2cm, minimum height=1.0cm, text width=7.0cm},
...
\node[mac, below=of M] (PC) {%
  ...
  TW-Rescue $\cdot$ Pool-Recombine $\cdot$ Route-Reduce};
...
\node[alns, below=of OC] (AE) {%
  ...
  2-opt / Or-opt polish\enspace$\cdot$\enspace ejection chains at $NV{=}NV_{\mathrm{BKS}}{+}1$};

\node[lac, below=of AE] (LA) {%
  \textbf{Learned Acceptance Criterion}\enspace(MLP $9{\to}48{\to}24{\to}1$)\\[3pt]
  \textit{In:}\enspace $\bigl[\Delta c/c_t,\ T_t/T_0,\ \texttt{no\_imp}/P_{\mathrm{stop}},\ \Delta NV,\
    t/T_{\max},\ \omega,\ \mathrm{util},\ \mathrm{slack},\ e^{-\Delta c/T_t}\bigr]$\\[1pt]
...
\node[eval, below=of LA] (EV) {%
  ...
  \scriptsize Phase 1: fleet-min\ $\lambda\!\in\!\{2,5,12,30,50\}{\times}\bar{c}$\enspace
  $\longrightarrow$\enspace Phase 2: distance-min ($\lambda{=}0$)};
...
\end{tikzpicture}
\caption{%
  Hybrid DDQN-ALNS architecture.
  Each block shows its \textit{In}/\textit{Out} interface.
  Solid arrows: forward data flow.
  Dashed arrows: reward feedback loops.
  Magenta arrow: goal-conditioning mode vector.}
...
A lightweight MLP ($9{\to}48{\to}24{\to}1$, sigmoid) replaces static
SA acceptance.  Nine features characterizing the candidate
solution---including cost delta, temperature ratio, fleet change, and
Metropolis probability---predict whether acceptance leads to an incumbent
improvement within horizon $H{=}80$.  Labels are assigned via hindsight:
\begin{equation}
  y_t = \begin{cases}
  ...
  \label{eq:lac}
\end{equation}
and the classifier is trained online with weighted binary cross-entropy.
