...
\begin{itemize}
    ...
    \item \textbf{Search Acceleration}: Across all tested instances, GNN-guided routing exhibits a significant runtime reduction (up to \textbf{86.6\% faster} on \texttt{C101}). This speedup is directly attributable to the \textbf{Dynamic Pruning Schedule}, which prunes low-probability local search moves early in the search loop, allowing the solver to conserve CPU resources for high-value neighborhoods.
