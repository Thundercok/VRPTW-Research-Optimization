...
def run_ortools(inst: Inst, limit: float, strategy_name: str) -> tuple[Plan | None, float]:
    ...
    # Cost Matrix (unbiased pure distance)
    ...
    # Map strategy name to proto enum
...
def main():
    ...
    strategies = [
        ...
        "GLOBAL_CHEAPEST_INITIAL_ROUTE",
        "FIRST_ACCEPT_HEURISTIC"
    ]
    
    for strategy in strategies:
        print(f"\n--- Strategy: {strategy} (15s Limit) ---")
        plan, t = run_ortools(inst, 15.0, strategy)
        if plan is not None:
            print(f"  Result: NV={plan.nv}, TD={plan.cost:.2f} ({t:.1f}s)")
        else:
            print("  Result: Failed")
