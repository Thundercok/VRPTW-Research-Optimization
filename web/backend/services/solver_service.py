...
def _build_dist(points, matrix):
    ...
    if matrix and len(matrix) == n and all(len(r) == n for r in matrix):
        return matrix
    return [
        [distance_km((points[i].lat, points[i].lng), (points[j].lat, points[j].lng)) for j in range(n)]
        for i in range(n)
    ]
...
def _shaw_removal(sol, k, rng):
    ...
    while len(removed) < min(k, len(all_c)):
        ...
        for c in all_c:
            if c in rs:
                continue
            score = 0.6 * sol.dist[seed][c] / max_dist + 0.4 * abs(sol.demands[seed] - sol.demands[c]) / max(
                sol.capacity, 1
            )
            if score < best_score:
...
def _greedy_insert(routes, removed, dist, demands, capacity, vehicles):
    ...
    for c in removed:
        ...
        for ri, route in enumerate(routes):
            ...
            if p is not None and d < best_d:
                best_d, best_ri, best_p = d, ri, p
        if best_ri >= 0:
            routes[best_ri].insert(best_p, c)
        elif len(routes) < vehicles:
            routes.append([c])
        else:
...
def _regret2_insert(routes, removed, dist, demands, capacity, vehicles):
    ...
    while remaining:
        ...
        if chosen_ri is not None:
            routes[chosen_ri].insert(chosen_p, chosen_c)
        elif len(routes) < vehicles:
            routes.append([chosen_c])
        else:
            least = min(range(len(routes)), key=lambda i: sum(demands[x] for x in routes[i]))
        ...
        for si, src in enumerate(best.routes):
            ...
            for sp, c in enumerate(src):
                ...
                for di, dst in enumerate(best.routes):
                    if di == si:
                        continue
                    if best.route_load(dst) + best.demands[c] > best.capacity:
                        continue
...
def _alns(initial, dist, demands, capacity, vehicles, iterations, seed=42):
    ...
    for _ in range(iterations):
        ...
        if d_idx == 0:
            nr, removed = _random_removal(sol, k, rng)
        elif d_idx == 1:
            nr, removed = _worst_removal(sol, k)
        else:
            nr, removed = _shaw_removal(sol, k, rng)

        r_idx = 0 if rng.random() * sum(rw) <= rw[0] else 1
        repaired = (_greedy_insert if r_idx == 0 else _regret2_insert)(nr, removed, dist, demands, capacity, vehicles)
...
async def solve_model(
    ...
    alns_sol = _alns([r[:] for r in nn], dist, demands, capacity, vehicles, iterations=min(600, max(200, n * 10)))
