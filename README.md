...
4. Kỳ vọng:
- Có toast `Import Successful`.
- Bảng `Customer List` thêm dòng mới đúng cột.
5. Nhấn `Run Model` và kiểm tra có kết quả ở tab Results.

## Lưu ý

- File `logs/results-v9.5/benchmark_clean.csv` là file benchmark tổng hợp, không phải file customer input để chạy model.
